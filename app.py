...
def make_prediction_and_chart(symbol):
    """Generate 7-day forecast using Linear Regression and return chart data."""
    try:
        ...
        hist_plot = close[-30:] if len(close) >= 30 else close
        
        hist_dates = [d.strftime('%Y-%m-%d') for d in hist_plot.index]
        hist_values = [round(float(v), 2) for v in hist_plot.values]
        
        pred_dates = [(hist_plot.index[-1] + timedelta(days=i+1)).strftime('%Y-%m-%d') for i in range(7)]
        pred_values = [round(float(p), 2) for p in preds]

        return hist_dates, hist_values, pred_dates, pred_values
...
def portfolio():
    ...
    portfolio_data, display, charts_data = {}, {}, {}
    total_cost, total_current = 0.0, 0.0

    for inv in holdings_db:
        sym = inv.symbol
        portfolio_data.setdefault(sym, {'shares': 0.0, 'cost': 0.0})
        portfolio_data[sym]['shares'] += inv.shares
        portfolio_data[sym]['cost'] += inv.shares * inv.buy_price
